...
const MeetingRoomCalendar = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get("https://localhost:7177/api/booking/events");
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      await axios.post("https://localhost:7177/api/booking/book", bookingData);
  ...
  const checkAvailability = async (start, end) => {
    try {
      const response = await axios.get(
        `https://localhost:7177/api/booking/availability?start=${moment(start).format("YYYY-MM-DD HH:mm")}&end=${moment(end).format("YYYY-MM-DD HH:mm")}`
      );
